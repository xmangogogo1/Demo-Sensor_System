...
import components.FireSensor;
import components.SecuritySensor;
...
public class SoSafe extends Observable{
    ...
    public boolean allControlPattern;//soSafeSystem get command to turn on all sensors each time
    public boolean timeControlPattern;//soSafeSystem get command to turn on both f and s sensors each time by room
    ...
    private Date soSafeInitialTime;// soSafe soSafeSystem install time
    ...
    public static class Alarm {
        ...
        public Alarm(Sensor sensor, int sensorId, int areaID){
            ...
            timer = new Timer();
            // add this alarm timer to the soSafeSystem timer list, which could holds multiple alarms at the same time
            timers.add(timer);
    ...
    public Sensor generateRandomSensor() {
        ...
        //if only turn on fire soSafeSystem
...
//                        temp.getAtArea());
     ...
     *****************************************************************************************/
    // Add the area which is ready to install any soSafeSystem into areaList of Area
    ...
    public void fireSystemInstall(int areaID, int fSensorNum) {
        ...
        if (fSensorNum != 0) {
            ...
            //save the fire soSafeSystem initial time
    ...
    //get the area that is chosen and highlighted to install sensor soSafeSystem
    public List<Area> getSelectArea() {return areaList; }
    //get the area ID that is chosen and highlighted to install sensor soSafeSystem
    ...
    public int getInstallFee() {
        ...
        //if user has already installed fire sensor soSafeSystem,  get 20% off of the security soSafeSystem install fee
        if (fSensorList.size() != 0 && sSensorList.size() != 0) {
            //soSafeSystem installation fee
            total += securityInstallationFee + .8 * fireInstallationFee;
            //total fee is soSafeSystem fee plus all sensors' fee
            total += fireSensorFee * fSensorList.size() + securitySensorFee * sSensorList.size();
            //output the soSafeSystem initial fee and record
            getInitailCharge(ssystemInitialTime,"Fire soSafeSystem ", "Security System ", total,fSensorList.size() ,sSensorList.size());
        }
        //if user only install fire soSafeSystem
        else if (sSensorList.size() == 0) {
            //total fee is soSafeSystem fee plus all sensors' fee
            total += fireInstallationFee + fireSensorFee * fSensorList.size();
            //output the soSafeSystem install fee and record
            getInitailCharge(fsystemInitialTime,"Fire soSafeSystem ", "", total, fSensorList.size(), 0);
        }
        //if user only install security soSafeSystem
        else {
            //total fee is soSafeSystem fee plus all sensors' fee
            total += securityInstallationFee + securitySensorFee * sSensorList.size();
            //output the soSafeSystem install fee and record
            getInitailCharge(ssystemInitialTime,"", "Security  soSafeSystem ", total, 0, sSensorList.size());
        }
        return total;
    }

    //Output the soSafeSystem installation fee and sensors installation fee
...
//        System.out.println(file.getAbsolutePath());
    ...
    public void getChargeRecord(){
        ...
        //if today is within soSafeSystem initial month, print initial fee
